...
import { generateMercadoPagoVerifierCircuitInputs } from "../helpers";
...
describe("Mercadopago email test", function () {
  ...
  beforeAll(async () => {
    rawEmail = fs.readFileSync(
      path.join(__dirname, "./emls/email-mercadopago.eml"),
      "utf8"
    );

    circuit = await wasm_tester(path.join(__dirname, "../src/mercadopago.circom"), {
  ...
  it("should verify mercadopago email", async function () {
    const twitterVerifierInputs = await generateMercadoPagoVerifierCircuitInputs(rawEmail, ethAddress);
    ...
    const dkimResult = await verifyDKIMSignature(rawEmail, "ddfundacion.org");
  ...
  it("should fail if the mercadopago receipt is invalid", async function () {
    const twitterVerifierInputs = await generateMercadoPagoVerifierCircuitInputs(rawEmail, ethAddress);
    twitterVerifierInputs.mercadoPagoUsernameIndex = (Number((await twitterVerifierInputs).mercadoPagoUsernameIndex) + 1).toString();
